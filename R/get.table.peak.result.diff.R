...
.get.table.peak.result.diff <- function(PEAK,ANNOTATION,READS_COUNT,SAMPLE_ID,PARAMETERS,
  ...
  for (i in 1:no_peak) {
    ...
    fold_enrchment=exp(median(PEAK$PW$log.fc[peak_row_id[1]:peak_row_id[2]]))
    untreated_folden=exp(median(PEAK$PW$log.un.fc[peak_row_id[1]:peak_row_id[2]]))
    treated_folden=exp(median(PEAK$PW$log.tr.fc[peak_row_id[1]:peak_row_id[2]]))
    ...
    xls=data.frame(chr=anno$chr,
             ...
             untreated_folden=untreated_folden,
             treated_folden=treated_folden)
