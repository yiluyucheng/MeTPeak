.metpeak.process2 <- function(IP,INPUT,batch_id,minimal_counts_in_fdr=10){
#   using constrained Newton's method written in c++ to call peaks per gene
  print("Peak Calling Stage! CPP")
  Ng = unique(batch_id)
  nip <- ncol(IP)
  nin <- ncol(INPUT)
...
ip_mean = mean(IP_mean) #averaged reads per nucleid
  ...
  for (ii in Ng){
    ...
    if (sum(ip)<ip_mean*nip){
      ...
      # using constrained Newton written in C++ to do the peak calling
      res = cmpHmm(ip,input+1,matrix(1/K,K,K),ab,F,h) #res[[2]] = trans; to avoid 0, input + 1;
      # decode the stat with viterbi algorithms
